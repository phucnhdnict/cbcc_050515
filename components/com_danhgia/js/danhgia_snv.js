...
var tinhphantramthoigian = function(){
	...
	var dtb_ngay = 8 * jQuery('input[name="float_diemchinh[]"]').eq(1).val()/jQuery('#thoigiandot').val();
	var diem_ngaynghi = 0;
	jQuery('select[name="loaingaynghi_id[]"]').each(function(){
		var row_index = jQuery('select[name="loaingaynghi_id[]"]').index(jQuery(this));
		diem_ngaynghi += parseInt(jQuery('input[name="songaynghi[]"]').eq(row_index).val()) * jQuery(this).find('option:selected').data('titrong') * dtb_ngay / 100;
	});
	diemphantramthoigian += diem_ngaynghi;
	if(phantramthoigian >= 100 || diemphantramthoigian >= jQuery('#label-diemphantramthoigian').data('diem_max')){
...
var showTongthoigian = function(){
	...
	var tongthoigian_ngaynghi = 0;
	jQuery("input[name='songaynghi[]']").each(function(i){
		tongthoigian_ngaynghi += jQuery(this).val() * 8;
	});

	if((tongthoigian_quydinh + tongthoigian_ngaynghi) > jQuery('#thoigiandot').val()){
		jQuery("#div-thoigianquydinh").html('Tổng thời gian sử dụng để làm việc trong giờ hành chính và số ngày nghỉ lớn hơn số ngày làm việc theo quy định trong tháng (' + (jQuery('#thoigiandot').val()/8).toFixed(2) + ' ngày). Đề nghị điều chỉnh!');
	}else{
...
var checkNgaynghi = function(){
	var elLoaingaynghi = jQuery("select[name='loaingaynghi_id[]']");
	var elSongaynghi = jQuery("input[name='songaynghi[]']");
	var elLydonghi = jQuery("textarea[name='lydonghi[][]']");
	var lengthEl = elLoaingaynghi.length;
	var flag = true;
	for(var i=0;i<lengthEl;i++){
		flag = flag && ((elLoaingaynghi.eq(i).val()=='')?false:true);
		flag = flag && ((elSongaynghi.eq(i).val()=='')?false:true);
		flag = flag && ((elLydonghi.eq(i).val()=='')?false:true);
		if(elLoaingaynghi.eq(i).val() == ''){
			elLoaingaynghi.eq(i).addClass('error');
		}
		if(elSongaynghi.eq(i).val() == ''){
			elSongaynghi.eq(i).addClass('error');
		}
		if(elLydonghi.eq(i).val() == ''){
			elLydonghi.eq(i).addClass('error');
		}
    }
    return flag;
};
jQuery(document).ready(function($){
	...
	$('body').delegate('select[name="loaingaynghi_id[]"]', 'change', function(){
		if($('#template_type').val() == 'thammuu' || $('#template_type').val() == 'truongphong' || $('#template_type').val() == 'congchuc'){
			if($('select[name="loaingaynghi_id[]"] option:selected[value="'+$(this).val()+'"]').length > 1){
				loadNoticeBoard('Thông báo lỗi','Bạn đã chọn loại ngày nghỉ này. Vui lòng chọn loại ngày nghỉ khác!');
				$(this).val('');
			}
			showTongthoigian();
			tinhphantramthoigian();
	...
	$('body').delegate('input[name="songaynghi[]"]', 'blur', function(){
		if($('#template_type').val() == 'thammuu' || $('#template_type').val() == 'truongphong' || $('#template_type').val() == 'congchuc'){
			showTongthoigian();
			tinhphantramthoigian();
		}
	...
	$('body').delegate('#btn_add_ngaynghi', 'click', function(){
		if (checkNgaynghi() == false){
    		loadNoticeBoard('Thông báo lỗi','Vui lòng nhập đầy đủ thông tin ngày nghỉ.');
            return false;
        }
		var str = '<tr>';
			str+= '<td class="center" style="vertical-align:middle;">';
			str+= str_select_loaingaynghi;
			str+= '</td>';
			str+= '<td class="center" style="vertical-align:middle;">';
			str+= '<input type="text" name="songaynghi[]" value="" class="input-mini" style="text-align:center;" />';
			str+= '</td>';
			str+= '<td class="center" style="vertical-align:middle;">';
			str+= '<textarea name="lydonghi[]" class="noidung" style="width:97%;"></textarea>';
			str+= '</td>';
			str+= '<td class="center" style="vertical-align:middle;">';
	    	str+= '<span class="btn btn-mini btn-danger btn_remove" style="width:20px;">';
	    	str+= '<i class="icon-trash"></i>';
	    	str+= '</span>';
			str+= '</td>';
			str+= '</tr>';
	    $('#tblNgaynghi tbody').append(str);
		$("textarea").autosize();
	    return false;
	});
    ...
    $('body').delegate('.btn_save_continue', ace.click_event, function(){
    	if($('#danhgia_type').val() == 'tudanhgia'){
    		if($('#template_type').val() == 'thammuu' || $('#template_type').val() == 'truongphong'){
		        ...
		        if($("#div-thoigianquydinh").html() != ''){
		        	loadNoticeBoard('Thông báo lỗi',$("#div-thoigianquydinh").html());
		            return false;
		        }
